...
describe("MasterChef", () => {
  ...
  beforeEach(async () => {
    ...
    waggyToken = await WaggyToken.deploy(deployer.address, "0", "100000000");

    const MasterChef = await ethers.getContractFactory("MasterChef");
    masterChef = await MasterChef.deploy(waggyToken.address, deployer.address, "1000", "0");
  ...
  it("alice staking/leaveStaking", async () => {
    ...
    await masterChef.connect(alice).enterStaking(ethers.utils.parseEther("1000"));
    let waggyBalance = await waggyToken.balanceOf(alice.address);
    userInfo = await masterChef.userInfo(0, alice.address);
    expect(ethers.utils.formatEther(waggyBalance)).equal("0.0");
    expect(ethers.utils.formatEther(userInfo.amount)).equal("1000.0");
    ...
    await masterChef.connect(alice).leaveStaking(ethers.utils.parseEther("800"));
    waggyBalance = await waggyToken.balanceOf(alice.address);
    userInfo = await masterChef.userInfo(0, alice.address);
    expect(ethers.utils.formatEther(waggyBalance)).equal("800.0");
    expect(ethers.utils.formatEther(userInfo.amount)).equal("200.0");
  });
});
