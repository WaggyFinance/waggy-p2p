...
contract Validator is Ownable{
    ...
    constructor(address _usdtAddress,uint256 _maxPercentValue, uint256 _minPercentValue) {
    ...
    function getCaseTotalValue(uint256 _caseId) external view returns(uint256){
    ...
    function getUserDecision(uint256 _caseId,address _owner) external view returns(bool){
        CaseInfo storage caseInfo = casesInfo[_caseId];
        return (caseInfo.usersReplyAnswer[_owner].createdAt != 0);
    }

    function evaluate(uint256 _caseId,bytes32 _randomness) external onlyOwner returns(bytes32){
        CaseInfo storage caseInfo = casesInfo[_caseId];
        require(caseInfo.result[0] == 0,"This case already had result.");
        uint256 buyyerValueCount;
        uint256 sellerValueCount; 
        for(uint i = 0; i< caseInfo.users.length;i++){
    ...
    function play(uint256 _caseId, uint256 _amount, bytes32 _answer ,string memory _remark) external {
        CaseInfo storage caseInfo = casesInfo[_caseId];
...
// transfer 
        require(usdtInterface.transferFrom(msg.sender, address(this), _amount),"Can't transfer");
// save reply
